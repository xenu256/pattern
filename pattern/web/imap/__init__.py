...
# Import the Cache class from pattern.web so e-mails can be cached locally
# (faster):
try:
...
class Mail(object):
    ...
    def folders(self):
        ...
        if self._folders is None:
            ...
            self._folders = [(_basename(f), MailFolder(self, f))
...
class MailFolder(object):
    ...
    def __getitem__(self, i, attachments=False, cached=True):
        ...
        if cached and id in cache:
            ...
            status, response = self.parent.imap4.select(self._name, readonly=1)
            status, response1 = self.parent.imap4.fetch(
                str(i), '(BODY.PEEK[HEADER])')
            status, response2 = self.parent.imap4.fetch(
                str(i), '(BODY.PEEK[%s])' % (not attachments and "TEXT" or ""))
            time.sleep(0.1)
