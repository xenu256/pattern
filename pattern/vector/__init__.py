...
def bin(iterable, key=lambda x: x, value=lambda x: x):
    """Returns a dictionary with items in the given list grouped by the given
    key."""
...
#--- WORD COUNT ----------------------------------------------------------
# Simple bag-of-word models are often made up of word frequencies or
...
def words(string, filter=lambda w: w.strip("'").isalnum(), punctuation=PUNCTUATION, **kwargs):
    """Returns a list of words (alphanumeric character sequences) from the
    given string.
    ...
    string = re.sub(
        r"([a-z|A-Z])'(m|s|ve|re|ll|d)", "\\1 <QUOTE/>\\2", string)
    string = re.sub(
        r"(c|d|gl|j|l|m|n|s|t|un)'([a-z|A-Z])", "\\1<QUOTE/> \\2", string)
    words = (w.strip(punctuation).replace("<QUOTE/>", "'", 1)
...
def count(words=[], top=None, threshold=0, stemmer=None, exclude=[], stopwords=False, language=None, **kwargs):
    ...
    if top is not None:
        count = count.__class__(
            heapq.nsmallest(top, list(count.items()), key=lambda kv: (-kv[1], kv[0])))
    return count
...
# A Document can be part of a training model used for learning (i.e.,
...
class Document(object):
    ...
    def __init__(self, string="", **kwargs):
        ...
        # Another Document: copy words, wordcount, name and type.
            ...
            raise TypeError(
                "document string is not str, unicode, list, dict, Vector, Sentence or Text.")
        # Document ID, used when comparing objects.
        self._id = _uid()
        # Name that describes the document content.
        ...
        # Total number of words (minus stop words).
        self._count = None
    ...
    def term_frequency_inverse_document_frequency(self, word, weight=TFIDF):
        ...
        if self.model is not None and weight == TFIDF:
            # Use tf if no model, or idf==None (happens when the word is not in
            # the model).
    ...
    def vector(self):
        ...
        if not self._vector:
            ...
            # When a document is added/deleted from a model, the cached vector
            # is deleted.
            w = getattr(self.model, "weight", TF)
            if w not in (TF, TFIDF, IG, INFOGAIN, GR, GAINRATIO, BINARY):
                f = lambda w: float(self._terms[w])
                w = None
            if w == BINARY:
    ...
    @property
    def concepts(self):
        """Yields the document concept vector if the document is part of an LSA
        model."""
    ...
    def copy(self):
        d = Document(
            None, name=self.name, type=self.type, description=self.description)
...
class Vector(readonlydict):
    ...
    def __call__(self, vector={}):
        """Vector(vector) returns a new vector updated with values from the
        given vector.
...
# For more information, see Domain Similarity Measures (Vincent Van Asch,
# 2012).
...
def distance(v1, v2, method=COSINE):
    ...
    # Squared Euclidean distance is used (1.5x faster).
    if method == EUCLIDEAN:
    ...
    if method == HAMMING:
        d = sum(
            not (w in v1 and w in v2 and v1[w] == v2[w]) for w in set(chain(v1, v2)))
...
# The matrix is used to calculate adjusted weights (e.g., tf * idf),
# document similarity and LSA.
...
class Model(object):
    ...
    def __init__(self, documents=[], weight=TFIDF):
        ...
        # Cache of ((d1.id, d2.id), relevance)-items (cosine similarity).
        self._cos = {}
        ...
        # Cache of (word, information gain ratio)-items.
        self._gr = {}
        self._inverted = {}             # Cache of word => Document.
        # Cache of model vector with all the features in the model.
        self._vector = None
        # Classifier trained on the documents in the model (NB, KNN, SVM).
        self._classifier = None
        self._lsa = None           # LSA matrix with reduced dimensionality.
        # Weight used in Document.vector (TF, TFIDF, IG, BINARY or None).
        self._weight = weight
        self._update()
    ...
    @classmethod
    def load(cls, path):
        """Loads the model from a gzipped pickle file created with
        Model.save()."""
    ...
    def save(self, path, update=False, final=False):
        ...
        The advantage is that cached vectors and cosine similarity are
        stored.
    ...
    def export(self, path, format=ORANGE, **kwargs):
        ...
        e.g., Orange or Weka."""
        # The Document.vector space is exported without cache or LSA concept
        # space.
    ...
    def document(self, name):
        """Returns the Document with the given name (assuming document names
        are unique)."""
    ...
    def inverse_document_frequency(self, word, base=2.71828):
        """Returns the inverse document frequency for the given word or
        feature.
    ...
    def vector(self):
        ...
        if not self._vector:
            self._vector = Vector(
                ((w, 0.0) for w in chain(*(d.terms for d in self.documents))), sparse=False)
        return self._vector
    ...
    def frequent_concept_sets(self, threshold=0.5):
        """Returns a dictionary of (set(feature), frequency) of feature
        combinations with a frequency above the given threshold."""
    ...
    def cosine_similarity(self, document1, document2):
        ...
        if not getattr(self, "lsa", None):
            ...
            v1 = id1 in self.lsa and self.lsa[
                id1] or self._lsa.transform(document1)
            v2 = id2 in self.lsa and self.lsa[
                id2] or self._lsa.transform(document2)
            s = cosine_similarity(v1, v2)
...
#   def cluster(self, method=HIERARCHICAL, k=1, iterations=1000)
    def cluster(self, method=KMEANS, **kwargs):
        """Clustering is an unsupervised machine learning method for grouping
        similar documents.
        ...
        if not getattr(self, "lsa", None):
            ...
            vectors, features = [
                d.vector for d in documents], list(self.vector.keys())
        else:
            # Using LSA concept space:
            vectors, features = [self.lsa[d.id]
                                 for d in documents], list(range(len(self.lsa)))
    ...
    def latent_semantic_analysis(self, dimensions=NORM):
        """Creates LSA concept vectors by reducing the vector space's
        dimensionality.
    ...
    def condensed_nearest_neighbor(self, k=1, distance=COSINE):
        """Returns a filtered list of documents, without impairing
        classification accuracy.

        Iteratively constructs a set of "prototype" documents. Documents
        that are correctly classified by the set are discarded.
        Documents that are incorrectly classified by the set are added
        to the set.
        ...
        while not b:
            ...
            for i, x in enumerate(v):
                nn = heapq.nsmallest(
                    k, ((d(x.vector, y.vector), y) for y in u))
    ...
    def posterior_probability(self, word, type):
        """Returns the probability that a document with the given word is of
        the given type."""
        if not self._pp:
            # p1: {class: count}
    ...
    def chi_squared(self, word):
        ...
        if not self._x2:
            ...
            for f in p3:
                p4[f] = chi2(
                    observed=[[p2[t][f] for t in p2], [p1[t] - p2[t][f] for t in p2]])[1]
            self._x2 = p4
    ...
    def information_gain(self, word):
        ...
        if not self._ig:
            ...
            F = set(self.features)
            V = dict((f, defaultdict(lambda: defaultdict(lambda: 0)))
                     for f in F)
            for d in self.documents:
                ...
                # the code below is over a 1000x faster (less
                # dict.__getitem__).
            for f in F:
    ...
    def feature_selection(self, top=100, method=CHISQUARED, threshold=0.0, weighted=False):
        """Returns a list with the most informative features (terms), using
        information gain.
    ...
    def filter(self, features=[], documents=[]):
        """Returns a new Model with documents only containing the given list of
        features, for example a subset returned from
    ...
    def train(self, *args, **kwargs):
        ...
        Model.predict() can then be used to predict the type of other
        (unknown) documents.
    ...
    def predict(self, *args, **kwargs):
        """Returns the type for a given document, based on the similarity of
        documents in the trained Model.classifier."""
...
class Apriori(object):
    ...
    def C1(self, sets):
        """Returns the unique features from all sets as a list of (hashable)
        frozensets."""
...
class LSA(object):
    ...
    def __init__(self, model, k=NORM):
        ...
        based on singular value decomposition (SVD), and related to principal
        component analysis (PCA).

        Closely related features (words) in the model are combined into
        "concepts". Documents then get a concept vector that is an
        approximation of the original vector, but with reduced
        dimensionality so that cosine similarity and clustering run
        faster.
        ...
        # u has document rows and concept columns, vt has concept rows and term
        # columns.
        ...
        self.model = model
        # Vt-index => word.
        self._terms = dict(enumerate(model.vector().keys()))
        self.u, self.sigma, self.vt = (
            dict((d.id, Vector((i, float(x)) for i, x in enumerate(v)))
                 for d, v in zip(model, u)),
            list(sigma),
    ...
    def concepts(self):
        ...
        """
        # Round the weight so 9.0649330400000009e-17 becomes a more meaningful
        # 0.0.
    ...
    def transform(self, document):
        """Given a document not in the model, returns a vector in LSA concept
        space.

        This happes automatically in Model.cosine_similarity(), but it
        must be done explicitly for Classifier.classify() input.
        ...
        v = numpy.dot(
            numpy.dot(numpy.linalg.inv(numpy.diag(self.sigma)), self.vt), v)
...
# The k_means() and hierarchical() functions work with Vector objects or
# dictionaries.


def mean(iterable, length=None):
    """Returns the arithmetic mean of the values in the given iterable or
    iterator."""
    if length is None:
...
class DistanceMap(object):
    ...
    def distance(self, v1, v2):
        ...
        try:
            ...
            # Two Vector objects for which the distance has not been
            # calculated.
            d = self._cache[(v1.id, v2.id)] = distance(
                v1, v2, method=self.method)
        except AttributeError:
...
#--- K-MEANS -------------------------------------------------------------
# k-means is fast but no optimal solution is guaranteed (random
# initialization).
...
def k_means(vectors, k=None, iterations=10, distance=COSINE, seed=RANDOM, **kwargs):
    """Returns a list of k clusters, where each cluster is a list of vectors
    (Lloyd's algorithm).
    ...
    while not converged and iterations > 0 and k > 0:
        ...
        # We can exploit this to avoid costly distance() calls (up to 3x
        # faster).
        # "Relaxed" triangle inequality (cosine distance is a semimetric) 0.25-0.5.
        p = 0.5 * kwargs.get("p", 0.8)
        D = {}
        for i in range(len(centroids)):
            # center1–center2 < center1–vector + vector–center2 ?
        ...
        for i in range(len(clusters)):
            for v in clusters[i]:
                ...
                if nearest != i:  # Other cluster is nearer.
                    clusters[nearest].append(
                        clusters[i].pop(clusters[i].index(v)))
                    converged = False
...
def kmpp(vectors, k, distance=COSINE):
    ...
    for _ in range(int(k) - 1):
        ...
        # Perform a number of local tries so that y yields a small distance
        # sum.
        i = 0
        ...
        d = [min(d[i], distance(v, centroids[-1]))
...
#--- HIERARCHICAL --------------------------------------------------------
# Hierarchical clustering is slow but the optimal solution guaranteed in
# O(len(vectors) ** 3).
...
class Cluster(list):
    ...
    def __init__(self, *args, **kwargs):
        """A nested list of Cluster and Vector objects, returned from
...
def sequence(i=0, f=lambda i: i + 1):
    ...
    # We cannot use id(vector), since id() is only unique for the lifespan of
    # the object.
    while True:
        yield i
        i = f(i)
...
def hierarchical(vectors, k=1, iterations=1000, distance=COSINE, **kwargs):
    """Returns a Cluster containing k items (vectors or clusters with nested
    items).
...
# The default baseline (i.e., the default predicted class) is the most
# frequent class:
...
class Classifier(object):
    ...
    def __init__(self, train=[], baseline=MAJORITY, **kwargs):
        ...
        # Description of the dataset: author e-mail, etc.
        self.description = ""
        ...
        # In Pattern 2.6+, it is replaced with Classifier._test() once
        # instantiated:
    ...
    @property
    def binary(self):
        """Yields True if the classifier predicts either True (0) or False
        (1)."""
    ...
    def skewness(self):
        ...
        a = list(
            chain(*([i] * v for i, (k, v) in enumerate(self._classes.items()))))
    ...
    def train(self, document, type=None):
        ...
        A document can be a Document, Vector, dict, list or string. If
        no type is given, Document.type will be used instead.
    ...
    def classify(self, document, discrete=True):
        """Returns the type with the highest probability for the given
        document.
    ...
    def _vector(self, document, type=None, **kwargs):
        ...
        if isinstance(document, Document):
            ...
            if document.model and document.model.lsa:
                # LSA concept vector.
                return type, document.model.lsa[document.id]
    ...
    def confusion_matrix(self, documents=[]):
        ...
        """
        documents = [
            isinstance(d, Document) and (d, d.type) or d for d in documents]
...
class ConfusionMatrix(defaultdict):
    ...
    def __init__(self, classify=lambda document: True, documents=[]):
        """Returns the matrix of classes x predicted classes as a
        dictionary."""
    ...
    def auc(self, k=10):
        ...
        roc = sorted(roc)
        # Trapzoidal rule: area = (a + b) * h / 2, where a=y0, b=y1 and
        # h=x1-x0.
...
def gridsearch(Classifier, documents=[], folds=10, **kwargs):
    ...
    for p in product(*p):
        p = dict(p)
        s.append(
            (K_fold_cross_validation(Classifier, documents, folds, **p), p))
...
class NB(Classifier):
    ...
    def __init__(self, train=[], baseline=MAJORITY, method=MULTINOMIAL, alpha=0.0001, **kwargs):
        """Naive Bayes is a simple supervised learning method for text
        classification.

        Documents are classified based on the probability that a feature
        occurs in a class, (independent of other features).
    ...
    def train(self, document, type=None):
        ...
        # Calculate the probability of a feature occuring in a class (=
    ...
    def classify(self, document, discrete=True):
        ...
        If the classifier has been trained on LSA concept vectors you
        need to supply LSA.transform(document).
        ...
        for type in self._classes:
            if m == MULTINOMIAL:
                if not type in self._cache:  # 10x faster
                    self._cache[type] = float(
                        sum(self._likelihood[type].values()))
                d = self._cache[type]
            if m == BINOMIAL \
                    or m == BERNOUILLI:
        ...
        try:
            ...
            m = max(p.values())
            p = sorted((self._classes[type], type)
                       for type, g in list(p.items()) if g == m > 0)
...
class KNN(Classifier):
    ...
    def classify(self, document, discrete=True):
        ...
        D = ((distance(v1, v2, method=self.distance), type)
             for type, v2 in self._vectors)
        # Nothing in common if distance=1.0.
        D = ((d, type) for d, type in D if d < 1)
        ...
        try:
            ...
            m = max(p.values())
            p = sorted((self._classes[type], type)
                       for type, w in list(p.items()) if w == m > 0)
            p = [type for frequency, type in p if frequency == p[0][0]]
            return choice(p)
        except:
            return self.baseline
...
class IGTree(Classifier):
    ...
    def __init__(self, train=[], baseline=MAJORITY, method=GAINRATIO, **kwargs):
        """IGTREE is a supervised learning method where training data is
        represented as a tree ordered by information gain.

        A feature is taken to occur in a vector (1) or not (0), i.e.
        BINARY weight.
    ...
    def _tree(self, vectors=[], features=[]):
        ...
        # build IGTREE._tree(subset, features[1:]) and connect it to the
        # current node.
        for x in p:
    ...
    def _train(self):
        """Calculates information gain ratio for the features in the training
        data.

        Constructs the search tree.

        """
        m = Model((Document(set(v), type=type)
                   for type, v in self._vectors), weight=BINARY)
        f = sorted(m.features, key=getattr(m, self._method), reverse=True)
        sys.setrecursionlimit(max(len(f) * 2, 1000))
        self._root = self._tree([(v, type)
                                 for type, v in self._vectors], features=f)
...
class SLP(Classifier):
    ...
    def __init__(self, train=[], baseline=MAJORITY, iterations=1, **kwargs):
        ...
        """
        self._weight = defaultdict(
            dict)  # {class: {feature: (weight, weight sum, timestamp)}}
    ...
    def train(self, document, type=None):
        ...
        def _accumulate(type, feature, weight, i):
            ...
            # we keep a running sum and the iteration in which the sum was last
            # modified.
            w = self._weight[type]
...
class BPNN(Classifier):
    ...
    def __init__(self, train=[], baseline=MAJORITY, layers=2, iterations=1000, **kwargs):
        """Backpropagation neural network (BPNN) is a supervised learning
        method bases on a network of interconnected neurons inspired by an
        animal's nervous system (i.e., the brain)."""
        # Based on:
    ...
    def _weight_initialization(self, i=1, o=1, hidden=1, method=RANDOM, a=0.0, b=1.0):
        """Initializes the network with the given number of input, hidden,
        output nodes.
    ...
    def _propagate_backward(self, output=[], rate=0.5, momentum=0.1):
        """Propagates the output through the network and generates delta for
        hidden and output nodes.

        The learning rate determines speed vs. accuracy of the
        algorithm.
    ...
    def classify(self, document, discrete=True):
        ...
        if not self._trained:
            # Batch learning (we need to know the number of features in
            # advance).
            n = float(len(self.classes)) - 1
            H1 = list(sorted(self.features))
            # Class => float hash (0.0-1.0).
            H2 = dict((x, i / n) for i, x in enumerate(self.classes))
            # Class reversed hash.
            H3 = dict((i / n, x) for i, x in enumerate(self.classes))
            v = [([v.get(f, 0.0) for f in H1], [H2[type]])
                 for type, v in self._vectors]
            self._h = (H1, H2, H3)
            self._weight_initialization(
                i=len(H1), o=1, hidden=self._layers, a=0.0, b=1.0)
...
class SVM(Classifier):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self._kernel == "rbf":
            self._kernel = RBF
        Classifier.__init__(
            self, train=kwargs.get("train", []), baseline=MAJORITY)
    ...
    def _train(self):
        ...
        # List of vectors.
        M = [v for type, v in self._vectors]
        # Feature => integer hash.
        H1 = dict((w, i + 1) for i, w in enumerate(self.features))
        # Class => integer hash.
        H2 = dict((w, i + 1) for i, w in enumerate(self.classes))
        # Class reversed hash.
        H3 = dict((i + 1, w) for i, w in enumerate(self.classes))
        # Hashed vectors.
        x = [dict([(H1[k], v[k]) for k in v]) for v in M]
        # Hashed classes.
        ...
        # SVM.train() will remove the cached model (since it needs to be
        # retrained).
    ...
    def _classify(self, document, probability=False):
        ...
        v = self._vector(document)[1]
        v = dict(
            [(H1.get(k[1], k[0] + n + 1), v[k[1]]) for k in enumerate(v)])
        ...
        p = f([0], [v], M, o)
        # Note: LIBLINEAR does not currently support probabilities for
        # classification.
...
# As pointed out by Lars Buitinck, words + word-level bigrams with TF-IDF
# can beat the 90% boundary:
...
# In scikit-learn it will run faster (4 seconds <=> 20 seconds), see:
# http://goo.gl/YqlRa
...
class GeneticAlgorithm(object):
    ...
    def __init__(self, candidates=[], **kwargs):
        """A genetic algorithm is a stochastic search method  based on natural
        selection.

        Each generation, the fittest candidates are selected and
        recombined into a new generation. With each new generation the
        system converges towards an optimal fitness.
    ...
    def update(self, top=0.5, mutation=0.5):
        """Updates the population by selecting the top fittest candidates, and
        recombining them into a new generation."""
        # 1) Selection.
